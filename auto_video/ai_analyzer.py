...
class AIAnalyzer:
    ...
    def analyze_emotion_and_scene(self, script: str) -> Tuple[str, str]:
        """
        只分析情绪和场景，不分段（用于推荐BGM）
...
{script[:500]}"""
    ...
    def _generate_keywords_batch(self, segment_texts: List[str]) -> List[List[str]]:
        """
        为一批字幕片段生成英文关键词
...
..."""
    ...
    def analyze_with_zhipu(self, script: str) -> Dict[str, Any]:
        """
        使用智谱AI进行智能分析
...
{script}"""
    ...
    def recommend_bgm(self, script: str, emotion: str = None, scene_type: str = None) -> Dict[str, Any]:
        """
        使用智谱AI推荐背景音乐
...
{{"music_id": "数字ID", "title": "标题", "reason": "简短理由"}}"""
    ...
    def recommend_sound_effects(self, script: str, segments: List[Dict] = None) -> List[Dict[str, Any]]:
        """
        使用智谱AI推荐音效
